...
def prepare_testbench_data(model, proj_name):
    ...
    tb_input = rng.uniform(-1.0, 1.0, size=(TB_SAMPLES, 128)).astype(np.float32)
...
def build_autoencoder_model():
    ...
    add_dense_relu(model, 128, 128, 0)
    add_dense_relu(model, 128, 128, 1)
    add_dense_relu(model, 128, 128, 2)
    add_dense_relu(model, 128, 128, 3)
    add_dense_relu(model, 128, 8, 4)
    add_dense_relu(model, 8, 128, 5)
    add_dense_relu(model, 128, 128, 6)
    add_dense_relu(model, 128, 128, 7)
    add_dense_relu(model, 128, 128, 8)

    return model


def run_autoencoder_model(REUSE_FACTOR, PART, CLK_PERIOD, VSYNTH=True, COSIM=True):
    seed = 0
    np.random.seed(seed)
    tf.random.set_seed(seed)

    clock_tag = format_clock_tag(CLK_PERIOD)
    proj_name = shell_safe_name(f'autoencoder_rf{REUSE_FACTOR}_clk{clock_tag}')
