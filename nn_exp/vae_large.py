...
def prepare_testbench_data(model, proj_name):
    ...
    tb_input = rng.uniform(-1.0, 1.0, size=(TB_SAMPLES, 64)).astype(np.float32)
...
def build_vae_large_model():
    model = Sequential(name='vae_large')
    model.add(
        QActivation(
    ...
    add_dense_relu(model, 64, 128, 0)
    add_dense_relu(model, 128, 64, 1)
    add_dense_relu(model, 64, 12, 2)
    add_dense_relu(model, 12, 64, 3)
    add_dense_relu(model, 64, 128, 4)
    add_dense_relu(model, 128, 64, 5)

    return model


def run_vae_large_model(REUSE_FACTOR, PART, CLK_PERIOD, VSYNTH=True, COSIM=True):
    seed = 0
    np.random.seed(seed)
    tf.random.set_seed(seed)

    clock_tag = format_clock_tag(CLK_PERIOD)
    proj_name = shell_safe_name(f'vae_large_rf{REUSE_FACTOR}_clk{clock_tag}')
    output_dir = BASE_DIR / 'nn_prj' / proj_name
    result_dir = BASE_DIR / 'nn_result' / proj_name

    model = build_vae_large_model()
    ...
    hls_model = hls4ml.converters.convert_from_keras_model(
        ...
        # io_stream packs each dense output vector into one FIFO word. For this
        # network, the pre-activation 128-wide tensors exceed Vitis' 4096-bit
        # aggregate limit, so use io_parallel locally for this model.
        io_type='io_parallel',
