...
def run_dense_model(NUM_LAYERS, IN_SIZE, OUT_SIZE, BITS, INT, REUSE_FACTOR, PART, CLK_PERIOD, VSYNTH=True, COSIM=True):
    seed = 0
    np.random.seed(seed)
    tf.random.set_seed(seed)

    clock_tag = format_clock_tag(CLK_PERIOD)
    proj_name = f'dsp_in{IN_SIZE}_out{OUT_SIZE}'
    if NUM_LAYERS != 1:
        proj_name += f'_l{NUM_LAYERS}'
    proj_name += f'_rf{REUSE_FACTOR}_clk{clock_tag}'
    proj_name = shell_safe_name(proj_name)
    output_dir = BASE_DIR / 'dsp_prj' / proj_name
    result_dir = BASE_DIR / 'dsp_result' / proj_name

    model = Sequential()
    model.add(
        QActivation(quantized_bits(BITS, INT), name="input_quant", input_shape=(IN_SIZE,)),
    )
    for i in range(NUM_LAYERS):
        layer_in_size, layer_out_size = (IN_SIZE, OUT_SIZE) if i % 2 == 0 else (OUT_SIZE, IN_SIZE)
        model.add(
            QDense(
                layer_out_size,
                input_shape=(layer_in_size,),
                name=f'fc{i}',
...
def time_block(fn, *args, **kwargs):
    ...
    print(f'{fn.__name__} took {dt * 1e3:.2f} ms = {h}h {m:02d}m {s:06.3f}s')
    return out, dt
...
def parse_args():
    ...
    parser.add_argument('--in-size', type=int, required=True)
    parser.add_argument('--out-size', type=int, required=True)
    parser.add_argument('--bits', type=int, default=8)
    parser.add_argument('--int-bits', type=int, default=0)
    parser.add_argument('--reuse-factor', type=int, required=True)
    parser.add_argument('--part', default='xcve2802-vsvh1760-2MP-e-S')
    parser.add_argument('--clock-period', type=float, default=5.0)
    parser.set_defaults(vsynth=True, cosim=True)
